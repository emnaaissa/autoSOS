...
<body class="bg-gray-100 min-h-screen">
                        ...
                        <?php if (strtolower($row['statut']) === 'terminé'): ?>
                            <?php if (strtolower($row['p_statut'] ?? '') === 'payé'): ?>
                                ...
                                <?php if (empty($row['a_note'])): ?>
                                    <div class="mt-3">
                                        <button type="button" onclick="ouvrirAvis(<?= $row['id_intervention'] ?>)" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-lg text-sm font-bold inline-block transition shadow-md">
                            ...
                            <?php else: ?>
                                <div class="mt-4">
                                    <button type="button" onclick="ouvrirPaiement(<?= $row['id_intervention'] ?>)" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-bold inline-block transition shadow-md">
    ...
    <script>
        ...
        function ouvrirPaiement(idIntervention) {
            $('#modalPaiement').removeClass('hidden').addClass('flex');
            setTimeout(() => {
                $('#modalPaiementContent').removeClass('scale-95 opacity-0').addClass('scale-100 opacity-100');
            }, 10);

            $.ajax({
                url: 'get_payment_form.php',
                type: 'GET',
                data: {
                    id: idIntervention
                },
                success: function(html) {
                    $('#modalPaiementContent').html(html);
                }
            });
        }

        function fermerPaiement() {
            $('#modalPaiementContent').removeClass('scale-100 opacity-100').addClass('scale-95 opacity-0');
            setTimeout(() => {
                $('#modalPaiement').addClass('hidden').removeClass('flex');
            }, 200);
        }

        $(window).on('click', function(e) {
            if ($(e.target).is('#modalPaiement')) fermerPaiement();
        });


        function ouvrirAvis(idIntervention) {
            $('#modalAvis').removeClass('hidden').addClass('flex');
            setTimeout(() => {
                $('#modalAvisContent').removeClass('scale-95 opacity-0').addClass('scale-100 opacity-100');
            }, 10);

            $.ajax({
                url: 'get_avis_form.php',
                type: 'GET',
                data: {
                    id: idIntervention
                },
                success: function(html) {
                    $('#modalAvisContent').html(html);
                }
            });
        }

        function fermerAvis() {
            $('#modalAvisContent').removeClass('scale-100 opacity-100').addClass('scale-95 opacity-0');
            setTimeout(() => {
                $('#modalAvis').addClass('hidden').removeClass('flex');
            }, 200);
        }
    </script>

    <div id="modalPaiement" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-95 opacity-0" id="modalPaiementContent">
            <div class="p-10 text-center">
                <i class="fas fa-circle-notch animate-spin text-4xl text-indigo-600"></i>
            </div>
        </div>
    </div>

    <div id="modalAvis" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden transform transition-all scale-95 opacity-0" id="modalAvisContent">
            <div class="p-10 text-center">
                <i class="fas fa-star animate-pulse text-4xl text-yellow-500"></i>
            </div>
        </div>
    </div>
